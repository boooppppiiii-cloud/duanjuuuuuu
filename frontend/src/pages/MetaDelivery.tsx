...
export default function MetaDelivery({embedded=false}:{embedded?:boolean}){
 ...
 const chooseAndBuild=async()=>{
   ...
   try{
     if(localStatus==='offline'){showLocalAssistantAccessPrompt();return}
 ...
 return <div className={embedded?'meta-delivery':'workspace-page meta-delivery'}>{ctx}
  {!embedded&&<div className="page-heading meta-page-heading"><Typography.Title level={2}><Space><PlatformLogo platform="meta" size={27}/><span>Meta 官方投递</span></Space></Typography.Title><Space><Tag icon={<SafetyCertificateOutlined/>} color="green">官方规范 v260626</Tag><Tag color="green">本机生成</Tag></Space></div>}
  <Steps size="small" current={check?.ready?2:source?.ready?1:0} className="meta-steps" items={[{title:'选择素材'},{title:'填写信息'},{title:'生成文件夹'}]}/>
  <Form form={form} layout="vertical" initialValues={defaults}><div className="meta-workbench"><Card title={<span className="workflow-card-title"><b>01</b> 素材</span>}>
  ...
  <Card title={<span className="workflow-card-title"><b>02</b> 系列信息</span>}>
    <Form.Item name="series_slug" label="英文文件夹名"><Input placeholder="例如 boss-like-me"/></Form.Item>
    <Form.Item name="description" label="系列简介" extra={descriptionShortened?`已自动精简至 ${descriptionShortened.to} 个字符`:undefined} rules={[{required:true,message:'Meta 要求填写系列简介'},{max:META_DESCRIPTION_MAX,message:'Meta 系列简介最多 500 个字符'}]}><Input.TextArea rows={4} maxLength={META_DESCRIPTION_MAX} showCount/></Form.Item>
    ...
    <Form.Item name="genres" label="题材分类" rules={[{required:true}]}><Select mode="multiple" disabled={Boolean(drama?.genres.length)} options={genres.map(item=>({value:item,label:item}))}/></Form.Item>
    <Space size="large" wrap><Form.Item name="ai_content" valuePropName="checked" label="AI 标识"><Switch disabled={Boolean(drama)}/></Form.Item><Form.Item name="dubbed_content" valuePropName="checked" label="配音标识"><Switch disabled={Boolean(drama)}/></Form.Item></Space>
  ...
  <Card title="已生成文件夹" className="table-card">{packages.length?<Table rowKey="id" dataSource={packages} pagination={false} scroll={{x:760}} columns={[
