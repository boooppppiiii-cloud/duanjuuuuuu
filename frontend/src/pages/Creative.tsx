...
export default function Creative({embedded=false}:{embedded?:boolean}) {
  ...
  useEffect(() => { Promise.all([api.list(), api.clips(), api.quotas(), api.accounts(),api.hotNotes(true,'tiktok')]).then(([ds, cs, q, ac,hot]) => { const approved=cs.filter(x=>x.status==='approved');const requested=Number(params.get('drama'));const selectedDrama=ds.find(x=>x.id===requested)??ds[0];setDramas(ds); setClips(cs); setQuota(q); setAccounts(ac); setHotNotes(hot); if (selectedDrama) { setDramaId(selectedDrama.id); void loadBasemaps(selectedDrama.id);setClipId(approved.find(x=>x.drama_id===selectedDrama.id)?.id) } if (ac[0]) { setAccountId(ac[0].id); setAccountType(ac[0].account_type) } }).catch(e => msg.error(e.message)) }, [])
  const changeDrama = (id: number) => { setDramaId(id); setClipId(clips.find(x => x.drama_id === id&&x.status==='approved')?.id); void loadBasemaps(id) }
  ...
  return <div className={embedded?'publishing-inner':'workspace-page'}>{context}{!embedded&&<Typography.Title level={2}>标题文案与封面</Typography.Title>}
    ...
    <Card title="每条切片生成标题"><Space wrap><Select value={clipId} onChange={setClipId} options={clips.filter(x=>(!dramaId||x.drama_id===dramaId)&&x.status==='approved').map(x=>({value:x.id,label:`成品 #${x.id}`}))} placeholder="选择已终审成品" style={{width:200}} /><Button type="primary" disabled={!clipId} loading={busy} onClick={makeTitles}>调用模型生成 3 组候选</Button>{titleProvider&&<Tag color="green">来源：{titleProvider}</Tag>}</Space>{!clipId&&<Alert className="page-alert" type="info" showIcon message="请先在内容工厂终审并保存至少一个成品"/>}{contextUsed.length>0&&<Alert type="success" showIcon message={`已筛选并注入：${contextUsed.join('；')}`} />}{candidates.length?<div className="candidate-grid">{candidates.map((item,index)=><Card key={index} className={item.hit_words.length?'candidate-danger':''} title={`公式 ${item.formula}`} extra={item.hit_words.map(x=><Tag color="red" key={x}>{x}</Tag>)}><Input value={item.title} onChange={e=>editCandidate(index,{title:e.target.value})} aria-label={`候选 ${index+1} 标题`} /><Input.TextArea rows={4} value={item.caption} onChange={e=>editCandidate(index,{caption:e.target.value})} aria-label={`候选 ${index+1} 文案`} /><Input value={item.hashtags.join(' ')} onChange={e=>editCandidate(index,{hashtags:e.target.value.split(/\s+/).filter(Boolean)})} aria-label={`候选 ${index+1} 标签`} /><Button type="primary" disabled={!!item.hit_words.length} onClick={()=>adopt(item)}>采用</Button></Card>)}</div>:<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="等待生成真实模型候选"/>}</Card>
