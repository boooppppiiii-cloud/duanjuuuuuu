...
export default function ContentFactory(){
  ...
  return <div className="workspace-page factory-page">{holder}
    ...
    {drama&&<div className="factory-task-strip"><b>{drama.title}</b>{drama.genres.map(genre=><Tag key={genre}>{genre}</Tag>)}<span>原片 {drama.episode_count}/{drama.total_episode_count}</span>{drama.is_ai_generated&&<Tag color="purple">AI 内容</Tag>}{drama.is_dubbed_content&&<Tag color="blue">配音内容</Tag>}<span>已生成 {drama.generated_files.length}</span></div>}
