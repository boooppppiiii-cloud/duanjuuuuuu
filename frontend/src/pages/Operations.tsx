...
export default function Operations(){
  ...
  const cols=[
    ...
    {title:'平台',dataIndex:'platform',render:(x:string)=>x==='all'?<PlatformLogoGroup/>:<PlatformBadge platform={x}/>},
  ...
  return <div className="workspace-page">{holder}
    <Space className="page-heading" align="start" wrap><Typography.Title level={2}><Space><PlatformLogo platform="tiktok" size={27}/><span>运营台</span></Space></Typography.Title><Button icon={<LinkOutlined/>} onClick={()=>navigate('/matrix')}>管理平台连接</Button></Space>
    <Card title={<Space><PlatformLogo platform="tiktok"/><span>账号连接</span></Space>} className="table-card">
      {tiktok.length?tiktok.map(a=><div key={a.id} className="connection-row">
        <Space><Avatar src={a.avatar_url} icon={<PlatformLogo platform="tiktok" size={19}/>}/><div><b>{a.name}</b><div className="cell-sub">{a.platform_user_id||'尚未读取平台身份'}</div></div></Space>
      ...
      </div>):<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="还没有账号"><Button type="primary" onClick={()=>navigate('/matrix')}>添加并连接账号</Button></Empty>}
    </Card>
    <Card title="记录热点" className="form-card"><Form layout="inline" onFinish={create} initialValues={{platform:'tiktok',expires_at:dayjs().add(7,'day')}}><Form.Item name="content" label="内容 / 标签" rules={[{required:true,message:'请输入热点'}]}><Input placeholder="#暑期追剧"/></Form.Item><Form.Item name="platform" label="平台"><Select style={{width:160}} options={[{value:'all',label:<span className="platform-option"><PlatformLogoGroup/><span>全部平台</span></span>},...['tiktok','instagram','facebook','youtube'].map(value=>({value,label:<PlatformOption platform={value}/>}))]}/></Form.Item><Form.Item name="expires_at" label="失效日期" rules={[{required:true}]}><DatePicker/></Form.Item><Button type="primary" htmlType="submit">记录</Button></Form></Card>
