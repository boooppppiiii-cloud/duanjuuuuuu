...
export default function Engagement({embedded=false}:{embedded?:boolean}){
  ...
  const analyze=async(useAi:boolean)=>{try{setWorking(true);const result=await api.analyzeComments(selected.map(Number),useAi);msg.success(useAi?`已完成 ${result.analyzed} 条真实模型分析`:`已用本地规则分析 ${result.analyzed} 条`);setSelected([]);await load()}catch(e:any){msg.error(e.message)}finally{setWorking(false)}}
