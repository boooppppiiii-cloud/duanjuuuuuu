...
export default function DashboardPage(){
  ...
  return <div className="workspace-page overview-page">{holder}
    ...
    {radarEvents.length>0&&<section className="dashboard-radar-strip"><div className="dashboard-radar-head"><b>今日传播动态</b><Button type="link" onClick={()=>navigate('/radar')}>查看全部</Button></div><div className="dashboard-radar-events">{radarEvents.map(item=><button key={item.id} onClick={()=>navigate(`/radar/dramas/${item.drama_id}`)}>{item.thumbnail_url?<img src={item.thumbnail_url} alt=""/>:<RadarChartOutlined/>}<span><b>{item.drama_title}</b><small>{item.title}</small></span><Tag color={item.severity==='high'?'red':item.severity==='positive'?'green':'cyan'}>{item.severity==='high'?'风险':item.severity==='positive'?'占位':'机会'}</Tag></button>)}</div></section>}
