...
export type Metric = { id: number; date: string; views: number; likes: number; comments: number; followers:number; post_title: string; account_name: string; account_type: string; cover_fallback: boolean; clip_id: number | null }
...
export type AccountMatrixRow = { id:number;platform:string;name:string;account_type:string;status:string;strategy_id:number|null;posts_7d:number;published_total:number;failed_total:number;views_7d:number;likes_7d:number;comments_7d:number;followers:number;last_publish_at:string|null;last_error?:string;last_checked_at?:string|null;avatar_url?:string;profile_url?:string;capabilities?:string[];configured?:boolean }
...
export type SocialComment = { id:number;external_id:string;platform:string;account_id:number|null;video_id:string;video_title:string;author_name:string;author_handle:string;text_original:string;like_count:number;published_at:string|null;sentiment:string;user_status:string;keyword_category:string;keywords:string[];summary:string;ticket_type:string;severity:string;needs_human:boolean;status:string;suggested_replies:string[];analysis_source:string;reply_id:string;reply_text:string;replied_at:string|null;fetched_at:string }
...
export const api = {
  ...
  accountMedia: (id:number,limit=25) => request<{id:string;title:string;published_at:string|null;views:number;likes:number;comments:number;url:string}[]>(`/api/publish/accounts/${id}/media?limit=${limit}`),
