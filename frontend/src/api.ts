...
export type EpisodeAnalysis = { episode:string;duration:number;segment_count:number;segments:ScriptSegment[];high_energy:ScriptSegment[];sensitive:ScriptSegment[];summary?:string;window_count?:number;source_fingerprint?:string }
export type FactoryAnalysisStageTimings = {transcription_seconds:number;frame_extraction_seconds:number;ai_review_seconds:number;total_seconds:number}
export type FactoryAnalysis = { task_id?:number;status:'not_analyzed'|'queued'|'processing'|'completed'|'failed';progress:number;current_step:string;error_message:string;drama_id:number;title:string;source?:string;provider?:string;model?:string;configured_provider?:string;configured_model?:string;ai_ready?:boolean;requires_reanalysis?:boolean;is_active?:boolean;generated_at?:string;updated_at?:string;resume_count?:number;reused_from_cloud?:boolean;shared?:boolean;shared_revision?:number;shared_updated_at?:string;cloud_sync_status?:'completed'|'failed';cloud_synced_at?:string;cloud_sync_error?:string;storage_mode?:string;source_fingerprint?:string;episode_count:number;completed_episode_count?:number;total_duration:number;segment_count:number;high_energy_count:number;sensitive_count:number;sampled_frame_count:number;api_call_count:number;cached_window_count?:number;completed_window_count?:number;total_window_count?:number;stage_timings?:FactoryAnalysisStageTimings;episodes:EpisodeAnalysis[] }
...
export type BriefingMedia = {account_id:number;account_name:string;platform:string;video_id:string;title:string;url:string;thumbnail_url:string;published_at:string;views:number;likes:number;comments:number;subscribers_gained?:number;views_per_hour?:number;engagement_rate?:number;metric_label?:string}
export type PlatformBriefing = {
  ...
  traffic_sources:{source:string;label:string;views:number;watch_time_seconds:number;share:number;range_start:string;range_end:string}[];
  week:{publish_count:number;active_accounts:number;views:number;likes:number;comments:number;average_views:number;engagement_rate:number;engagement_leader:BriefingMedia|null;subscriber_leader:BriefingMedia|null};
...
export const api = {
  ...
  mappedBatchPublish: (assignments:{post_id:number;account_ids:number[]}[],scheduledAt:string,runNow=false,aiDisclosure=false,publishOptions:Record<string,unknown>={}) => request<PublishJob[]>('/api/publish/jobs/mapped-batch',{method:'POST',body:JSON.stringify({assignments,scheduled_at:scheduledAt,run_now:runNow,ai_disclosure:aiDisclosure,publish_options:publishOptions})}),
