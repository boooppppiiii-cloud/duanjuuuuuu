...
2. **剧库 / 共享素材池**：只管理近期准备运营的剧目，不代替公司现有全量剧目后台；剧目资料共享，源视频默认通过本地工作区直接读取，不上传服务器。
3. **内容工厂**：本地工作区用 Whisper 与 FFmpeg 直接处理电脑里的原片，完成脚本、高能点、敏感内容和批量成品。原片与默认成品留在本机；可复用的识别结果同步到服务器。
...
使用 `https://app.duanju.chat` 时，先在存放本项目的电脑上双击 `start-local-workspace.bat`，保持窗口开启。随后在“内容工厂”点击“选择本地文件夹”，系统会弹出 Windows 文件夹选择窗口。

- 原始视频不会上传服务器，也不会复制到浏览器缓存。
- Whisper、抽帧和 FFmpeg 剪辑在当前电脑执行，默认成品写入源文件夹的 `generated/`。
- 服务器只保存剧目资料、文件清单、脚本、风险区间、高能点和任务用量记录。
- 同一剧目已有共享识别结果且文件名完全一致时，本机会直接复用，不再次调用模型。
- 本机工作区仅监听 `127.0.0.1:17862`，局域网和互联网无法直接访问。
...
| 本地加工任务、个人成品 | 操作员电脑的源文件夹 + `%LOCALAPPDATA%/Jushu/local-workspace/` | 原片不上传；默认成品位于源文件夹 `generated/`，本地任务库按电脑保存 |
| 发布草稿/任务、Meta 投递记录 | `app.db` + 用户明确提交的媒体 | 按登录账号隔离；只有进入发布或主动共享时才传输对应成品 |
| 云剧库成品 | `media/cloud-library/` | 仅用户主动上传后共享；同盘优先硬链接，不重复占用源成品磁盘块，跨盘才复制 |
| 原片 | 操作员电脑自选文件夹 | 只在本机读取，不占服务器容量；服务器只保留不含绝对路径的文件清单 |
| 封面、剧照 | `media/dramas/` | 体积较小，随剧目资料在服务器共享 |
