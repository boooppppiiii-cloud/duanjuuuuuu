...
'@ | Set-Content -LiteralPath $launcherFile -Encoding UTF8
    ...
    for ($attempt = 0; $attempt -lt 45; $attempt++) {
        ...
        try {
            $health = Invoke-RestMethod -Uri "http://127.0.0.1:17862/api/local/health" -TimeoutSec 2
            if ($health.status -eq "ok" -and $health.version -eq "1.7.0" -and $health.windows_user -eq $env:USERNAME -and $health.session_id -eq $currentSessionId -and $health.picker_ready -eq $true -and $health.capabilities -contains "native_folder_picker_v3" -and $health.capabilities -contains "legacy_server_import_v1" -and $health.capabilities -contains "factory_cancel_v1" -and $health.capabilities -contains "local_storage_manager_v1" -and $health.capabilities -contains "meta_duration_guard_v1") { $ready = $true; break }
        } catch { }
    }
