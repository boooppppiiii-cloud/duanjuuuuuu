...
try {
    ...
    if ($health.status -eq "ok" -and $health.version -eq "1.11.0" -and $health.windows_user -eq $env:USERNAME -and $health.session_id -eq $currentSessionId -and $health.picker_ready -eq $true -and $health.capabilities -contains "native_folder_picker_v3" -and $health.capabilities -contains "legacy_server_import_v1" -and $health.capabilities -contains "factory_cancel_v1" -and $health.capabilities -contains "factory_sensitive_policy_v2" -and $health.capabilities -contains "factory_analysis_worker_v1" -and $health.capabilities -contains "local_storage_manager_v1" -and $health.capabilities -contains "factory_model_proxy_v1" -and $health.capabilities -contains "meta_duration_guard_v1" -and $health.capabilities -contains "meta_metadata_guard_v1") { exit 0 }
} catch { }
