...
def test_manual_review_can_override_automatic_block(tmp_path: Path):
    ...
    with Session(engine) as session:
        ...
        clip = Clip(drama_id=drama.id, template_name="suspense_hook", status="blocked", current_step="completed", hit_words=["测试词"])
        session.add(clip); session.commit(); session.refresh(clip)
        result = review_clip(clip.id, ClipReviewRequest(status="approved", note="人工确认语境安全"), session)
