...
def login_test_user(client: TestClient) -> int:
    response = client.post("/api/auth/register", json={"email": "drama-tests@example.com", "password": "correct-horse-1"})
...
def test_direct_video_upload_and_manual_register(tmp_path: Path):
    ...
    with TestClient(app.main.app) as client:
        ...
        registered = client.post("/api/dramas/register", json={"title": "绝对路径剧", "theater": "FlickReels", "absolute_path": str(external), "source_note": "授权测试"})
        assert registered.status_code == 200, registered.text
        assert registered.json()["theater"] == "FlickReels"
...
def test_task_metadata_cover_upload_and_approved_asset_library(tmp_path: Path):
    ...
    with TestClient(app.main.app) as client:
        ...
        assert published_asset.json()["generated_files"] == []
        ...
        with Session(app.database.engine) as session:
            clip = Clip(drama_id=drama["id"], template_name="suspense_hook", current_step="completed", status="pending", file_path=str(final), owner_user_id=user_id)
        ...
        assert refreshed["generated_files"] == []
        assert any(item["id"] == clip_id and item["status"] == "approved" for item in client.get(f"/api/clips?drama_id={drama['id']}").json())
