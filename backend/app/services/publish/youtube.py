...
class YouTubeChannel(PublishChannel):
    def publish(self, payload: PublishPayload) -> PublishResult:
        ...
        body = {
            "snippet": {
                "title": payload.post.title[:100],
                "description": payload.post.caption,
                "tags": [item.lstrip("#") for item in payload.post.hashtags][:30],
                "categoryId": str(payload.account.credentials_json.get("category_id") or "24"),
            },
            "status": {
                "privacyStatus": privacy,
                "selfDeclaredMadeForKids": bool(options.get("made_for_kids", False)),
                "containsSyntheticMedia": bool(payload.job.ai_disclosure),
            },
        }
        request = service.videos().insert(
            part="snippet,status",
            body=body,
            media_body=MediaFileUpload(str(payload.video), mimetype="video/mp4", resumable=True),
        )
        ...
        cover_log = ""
        if payload.cover and payload.cover.is_file():
            try:
                service.thumbnails().set(
                    videoId=video_id,
                    media_body=MediaFileUpload(str(payload.cover), mimetype="image/jpeg", resumable=False),
                ).execute()
                cover_log = "；剧库封面已设置"
            except Exception as exc:
                cover_log = f"；视频已上传，但封面设置失败：{exc}"
        return PublishResult(
            success=True,
            video_id=video_id,
            log=f"YouTube 已接收视频，当前可见性：{privacy}{cover_log}",
            status="submitted",
