...
def _upsert_account(session: Session, platform: str, platform_id: str, name: str, credentials: dict, avatar_url: str = "", profile_url: str = "", followers: int = 0, capabilities: list[str] | None = None) -> Account:
    ...
    if not account:
        placeholders = session.exec(select(Account).where(Account.platform == platform, Account.platform_user_id == "", Account.removed_at.is_(None))).all()
    ...
    account.removed_at = None; account.last_error = ""; account.last_checked_at = datetime.now(); account.connected_at = account.connected_at or datetime.now()
